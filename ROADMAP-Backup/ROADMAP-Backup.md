...
## Non-Negotiable Page Flow Rule

Physical pages must never be implemented as internally scrollable text boxes.

In print-layout or WYSIWYG page mode:
- a page is a fixed physical container with a fixed writable frame,
- pasted or opened long text must create page 2, page 3, etc.,
- text may continue into the next column or next page, but it must not disappear
  below the page,
- the only normal scrollbar should belong to the document/page canvas,
- individual page widgets must have their internal scrollbars disabled or unused,
- the full document text must remain in the document model, not only in the first
  visible page widget.

Implementation warning:
- Do not treat `QTextEdit`, HTML `textarea`, or similar widgets as the document
  pagination engine.
- They may be used as editing surfaces only if overflow is intercepted and moved
  into the application's page/column layout model.
- Pagination must be verified by measuring actual rendered content height/width
  using font metrics or layout APIs, not only by counting newline characters.
...
Page widgets are visual containers, not infinite editors. A page may show a text
editing widget internally, but that widget must not own overflow behavior. If
content no longer fits inside the writable frame, the document layout engine must
create another page or column and continue the flow there.
...
Critical rules:
- Opening a long file must immediately paginate it into as many pages as needed.
- Pasting long text must immediately paginate it into as many pages as needed.
- Changing page size, orientation, margins, zoom, font, or column settings must
  invalidate layout and recalculate pagination.
- Page-local editor scrollbars must be disabled in print-layout mode.
- Overflow detection must use actual rendered layout height/width, including word
  wrapping, font metrics, document margins, line spacing, and chord/lyric rows.
- The document model must store the complete source text independently from the
  currently visible page widgets.
- Tests must assert that long documents create extra pages and that no page
  contains hidden overflowing text.

- [ ] Automatic page overflow handling
- [ ] Automatic page creation
- [ ] Text reflow between pages
- [ ] Create pages automatically when opening long text
- [ ] Create pages automatically when pasting long text
- [ ] Disable or avoid internal scrollbars inside physical page widgets
- [ ] Preserve full document text while displaying it across multiple pages
- [ ] Measure real rendered content height before deciding that text fits
...
- [ ] Text reflow
- [ ] Opening long documents creates additional pages automatically
- [ ] Pasting long text creates additional pages automatically
- [ ] Page widgets do not scroll internally in print-layout mode
- [ ] Overflow tests cover wrapped lines, long paragraphs, and ChordPro rows
- [ ] Manual page breaks
- [ ] Zoom
...
- [ ] When importing many songs, long songs must create additional pages instead
      of overflowing inside a song/page widget
