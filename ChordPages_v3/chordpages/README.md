...
ChordPages is an early PyQt6 prototype for a page-oriented WYSIWYG ChordPro
...
- Loaded source text is split across physical pages instead of scrolling inside
  an individual page
...
ChordPages is currently an early source-run prototype. Packaged installers are not
...
```bash
git clone https://github.com/wachin/chordpages.git
cd chordpages
...
Run ChordPages from the repository root:
...
ChordPages keeps Qt Linguist source and compiled translation files under
`translations/`. Runtime startup loads `chordpages_<locale>.qm` with
...
```bash
pylupdate6 chordpages/ --ts translations/chordpages_es.ts
pylupdate6 chordpages/ --ts translations/chordpages_en.ts
```

Edit translations with Qt Linguist, then compile `.qm` files:

```bash
linguist-qt6 translations/chordpages_es.ts
lrelease translations/chordpages_es.ts -qm translations/chordpages_es.qm
lrelease translations/chordpages_en.ts -qm translations/chordpages_en.qm
```
