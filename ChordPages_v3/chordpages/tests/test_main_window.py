...
from chordpages.autosave import AutosaveManager
...
from chordpages.fonts import DEFAULT_EDITOR_FONT_SIZE_PT
from chordpages.i18n import install_translations
import chordpages.main_window as main_window_module
from chordpages.main_window import MainWindow
from chordpages.recovery import RecoveryManager
...
from chordpages.theme import CURRENT_THEME_PROPERTY, EFFECTIVE_THEME_PROPERTY
...
def test_settings_manager_persists_editor_font(tmp_path):
    settings = temporary_settings(tmp_path)
...
def test_main_window_starts_with_page_editor(qtbot, tmp_path):
    ...
    assert window.editor.page_layout().size_mm == (63.0, 110.0)
    assert window.editor.page_layout().margins == DEFAULT_PAGE_MARGINS
    assert window.editor.editor_font().pointSizeF() == pytest.approx(
        DEFAULT_EDITOR_FONT_SIZE_PT
    )
    assert window.editor.editor_font().fixedPitch()
...
def test_main_window_exposes_editor_font_controls(qtbot, tmp_path):
    settings = temporary_settings(tmp_path)
    window = MainWindow(settings=settings)
    qtbot.addWidget(window)

    assert isinstance(window.font_family_combo, QFontComboBox)
    assert isinstance(window.font_size_spin, QDoubleSpinBox)
    assert window.font_family_combo.objectName() == "fontFamilyComboBox"
    assert window.font_size_spin.objectName() == "fontSizeSpinBox"
    assert (
        window.font_family_combo.fontFilters()
        & QFontComboBox.FontFilter.MonospacedFonts
    )
    assert window.font_size_spin.value() == pytest.approx(
        DEFAULT_EDITOR_FONT_SIZE_PT
    )

    window.font_size_spin.setValue(8.5)

    assert window.editor.editor_font().pointSizeF() == pytest.approx(8.5)
    assert settings.editor_font_size(DEFAULT_EDITOR_FONT_SIZE_PT) == 8.5
    assert window.statusBar().currentMessage().startswith("Font:")


def test_main_window_applies_saved_editor_font_to_new_tabs(qtbot, tmp_path):
    settings = temporary_settings(tmp_path)
    settings.set_editor_font_family("DejaVu Sans Mono")
    settings.set_editor_font_size(7.5)
    settings.sync()
...
def test_main_window_switches_page_view_modes(qtbot, tmp_path):
    ...
    assert window.editor.pages_per_row() == 3
    assert window.multiple_page_view_action.isChecked()
    assert not window.single_page_view_action.isChecked()
...
def test_main_window_recovers_selected_autosave_draft(qtbot, monkeypatch, tmp_path):
    ...
    autosave_manager.write(
        ChordPagesDocument(text="[E]Recovered", title="Recovered"),
...
def test_main_window_discards_selected_autosave_draft(qtbot, monkeypatch, tmp_path):
    autosave_manager = AutosaveManager(tmp_path / "autosave")
    autosave_manager.write(
        ChordPagesDocument(text="[E]Discard me", title="Discard"),
        source_path=None,
        draft_id="draft-1",
    )
    recovery_manager = RecoveryManager(autosave_manager)
...
def test_main_window_opens_text_files_in_tabs_with_metadata(qtbot, tmp_path):
    path = tmp_path / "song.pro"
    path.write_bytes("C    G\r\nletra\r\n".encode("utf-8"))
    window = MainWindow(settings=temporary_settings(tmp_path))
    qtbot.addWidget(window)
    ...
    assert window.editor.text() == "C    G\nletra\n"
    assert window.current_document().file_type == "chordpro"
    assert window.current_document().encoding == "utf-8"
    assert window.current_document().newline == "\r\n"
    assert window.current_path == path.resolve(strict=False)


def test_main_window_preserves_text_file_line_endings_on_save(qtbot, tmp_path):
    source = tmp_path / "song.txt"
    source.write_bytes("C    G\r\nletra\r\n".encode("utf-8"))
    target = tmp_path / "saved.txt"
    window = MainWindow(settings=temporary_settings(tmp_path))
    qtbot.addWidget(window)
    window.load_path(source)
    window.editor.set_text("D    A\nletra\n")

    window.save_path(target)

    assert target.read_bytes() == b"D    A\r\nletra\r\n"
    assert window.current_document().file_type == "text"
    assert not window.current_document().modified


def test_main_window_transposes_current_tab_chords(qtbot, tmp_path):
    window = MainWindow(settings=temporary_settings(tmp_path))
    qtbot.addWidget(window)
    window.editor.set_text("[C]Vine [G]a adorar\nAm    F\nletra\n")

    window.transpose_current_document(2)

    assert window.editor.text() == "[D]Vine [A]a adorar\nBm    G\nletra\n"
    assert window.current_document().modified
