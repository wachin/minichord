...
def test_mchord_round_trip_preserves_text_and_title():
    document = MiniChordDocument(title="Vine a adorarte", text="[G]Vine a adorarte\n")

    loaded = MiniChordDocument.from_mchord(document.to_mchord())
...
def test_plain_text_loads_as_document_body():
    loaded = MiniChordDocument.from_mchord("plain song text")
